...
const swotItems = [
  { cat: 'Forces', color: '#34d399', items: ['Expertise métier solide', 'Marque reconnue', 'ISO 9001'] },
  { cat: 'Faiblesses', color: '#f87171', items: ['SI obsolète (ERP, Planning)', 'Excel central', 'Gouvernance IT faible'] },
  { cat: 'Opportunités', color: '#fb923c', items: ['Expansion internationale', 'Digitalisation marketing', 'Cloud & IA'] },
  { cat: 'Menaces', color: '#fbbf24', items: ['Cybermenaces', 'Non-conformité RGPD', 'Concurrence numérique'] },
]
    ...
    <SlideLayout accent="#e879f9">
      ...
      <div className="shrink-0 mb-5">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-stone-500 text-sm">Difficultés & Apprentissages</span>
        </div>
        <h2 className="text-3xl font-bold text-white">MSPR — Défis rencontrés</h2>
        <p className="text-stone-400 text-sm mt-1">Complexités inhérentes à la transformation SI</p>
      </div>
      ...
      <div className="flex-1 min-h-0 flex gap-5">
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3">
          {defis.map(d => (
            <div key={d.title} className="flex-1 min-h-0 bg-white/5 border border-fuchsia-500/20 rounded-2xl p-4 overflow-hidden">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <div className="mt-2 flex items-start gap-2">
                    ...
                    <p className="text-fuchsia-300 text-xs leading-relaxed">{d.solution}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="w-64 shrink-0 flex flex-col gap-3">
          ...
          <div className="flex-1 min-h-0 bg-white/5 border border-fuchsia-500/25 rounded-2xl p-4 overflow-hidden">
            <p className="text-xs text-fuchsia-400 font-semibold uppercase tracking-wider mb-2.5">SWOT K-Electronik</p>
              ...
              {swotItems.map(({ cat, color, items }) => (
                <div key={cat} className="p-2 rounded-xl border overflow-hidden" style={{ borderColor: `${color}40`, background: `${color}08` }}>
                  ...
                  <ul className="space-y-0.5">
                    {items.map(i => <li key={i} className="text-stone-400 text-xs">· {i}</li>)}
                  </ul>
                </div>
              ))}
            </div>
          ...
          <div className="shrink-0 bg-white/5 border border-fuchsia-500/25 rounded-2xl p-4">
            <p className="text-xs text-fuchsia-400 font-semibold uppercase tracking-wider mb-2.5">Compétences acquises</p>
            <div className="space-y-1.5">
              {learnedSkills.map(l => (
                <div key={l} className="flex items-center gap-2 text-xs text-stone-300">
                  <span className="text-fuchsia-400 shrink-0">✓</span> {l}
                </div>
              ))}
            </div>
          </div>
